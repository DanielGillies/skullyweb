		...
		return this.each(function() {
			...
			function onDocumentClick (e) {
				...
				if (options.closeByDocument) {
					if (!$(e.target).is('.btn') && !$(e.target).is('.sharebut') && !$(e.target).is('.ti-facebook2') && !$(e.target).is('.ti-twitter-alt2') && !$(e.target).is('.ti-google2')) {
						console.log("BREAK WINDOW");
					e.preventDefault();
						deactivate();
					}
					else if ($(e.target).is('.avgrund-overlay, .avgrund-close')) {
						e.preventDefault();
						deactivate();
					}
